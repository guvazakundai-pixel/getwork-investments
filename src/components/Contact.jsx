...
const contacts = [
  ...
  },
  {
    icon: MapPin,
    title: "Address",
    value: "Corner Speke and Mbuya Nehanda, Cyruss Mall, Opposite Chicken Slice, Second Floor, C16",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {contacts.map((item) => (
            ...
            >
              ...
              <p className="text-gray-500 text-sm mb-4">{item.value}</p>
              <a
                href={item.href}
                target={item.href.startsWith("http") ? "_blank" : undefined}
                rel={item.href.startsWith("http") ? "noopener noreferrer" : undefined}
                className="inline-block bg-royal-blue text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-dark-blue transition-colors"
              >
                {item.action}
              </a>
            </motion.div>
        ...
        >
          ...
          <a
            ...
            className="inline-flex items-center gap-2 bg-green-600 text-white px-8 py-3.5 rounded-full font-semibold text-sm hover:bg-green-700 transition-colors"
          >
            <svg
              className="w-5 h-5"
              fill="currentColor"
